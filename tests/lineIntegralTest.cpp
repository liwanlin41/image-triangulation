...
TEST(IntegralTest, Zero) {
    ...
    ASSERT_EQ(0, LineIntegral::evaluate(z, &image, &triangle));
    ASSERT_EQ(0, LineIntegral::evaluate(z, &image, &a, &b, &c));
}
...
TEST(IntegralTest, Constant) {
    ...
    Point c(0,4);
    Triangle triangle(&a, &b, &c);
    ASSERT_EQ(12, LineIntegral::evaluate(constant, &image, &triangle));
    ASSERT_EQ(12, LineIntegral::evaluate(constant, &image, &a, &b, &c));
}
...
TEST(IntegralTest, Linear) {
    ...
    Point c(0,4);
    Triangle triangle(&a, &b, &c);
    ASSERT_EQ(30, LineIntegral::evaluate(linear, &image, &triangle));
    ASSERT_EQ(30, LineIntegral::evaluate(linear, &image, &a, &b, &c));
}
